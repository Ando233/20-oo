...
public class Management {
    ...
    private int checkStations(String name){
        for(Map.Entry<String, Line> e : Lines.entrySet()){
            if(e.getValue().stations.containsKey(name)){
                return e.getValue().stations.get(name);
            }
        }
        return -1;
    }
    ...
    public void delLine(String ...args){
        ...
        for(Map.Entry<String, Train> e : line.trains.entrySet()){
    ...
    public void checkTicket(String ...args){
        ...
        if(trainId.charAt(0) == '0'){
            if(seat.equals("CC")){
                price = train.getPrices()[1];
                num = train.getNums()[1];
            }
            else if(seat.equals("SB")){
                price = train.getPrices()[2];
                num = train.getNums()[2];
            }
            else if(seat.equals("GG")){
                price = train.getPrices()[3];
                num = train.getNums()[3];
            }
            else {
